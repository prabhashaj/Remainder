...
export const ClaimItemSchema = z.object({
  ...
  source_title: z.string().describe("Title of primary supporting source"),
  source_url: z.string().describe("URL or identifier of the source"),
  exact_support: z.string().describe("Exact quote or summary from the source supporting this claim"),
  
  // Experimental & System Context
  model: z.string().optional().describe("Specific model/architecture evaluated (e.g. LLaMA-3-8B, GPT-4, ResNet-50)"),
  dataset: z.string().optional().describe("Specific benchmark or dataset evaluated (e.g. MMLU, GSM8K, ImageNet-1K)"),
  task: z.string().optional().describe("Task or domain of evaluation"),
  metric: z.string().optional().describe("Metric evaluated (e.g. accuracy, latency, FLOPs, perplexity)"),
  ...
  hardware: z.string().optional().describe("Hardware setup if relevant (e.g. 8x H100, A100-80GB)"),
  experimental_context: z.string().optional().describe("Full experimental context (batch size, precision, sequence length)"),
  evidence_level: z
    .enum([
      "theoretical",
      "simulation",
  ...
  // Rigor
  what_source_showed: z.string().optional().describe("What the paper/source actually demonstrated"),
  what_source_did_not_show: z.string().optional().describe("What the paper/source did NOT demonstrate or test"),
  ...
  verification_status: z.enum(["VERIFIED", "PARTIALLY_SUPPORTED", "UNSUPPORTED", "CONTRADICTED", "UNVERIFIED"]),
});
...
export function extractNumericTokens(text: string): string[] {
  if (!text) return [];
  // Match percentages (e.g. 97.2%, 45%), multipliers (3.2x, 3.2×), raw decimals/integers with units (12ms, 120GB, 7B, 70B)
...
export function verifyNumericalClaim(
  ...
  for (const figure of claimFigures) {
    // Normalization check (e.g. 97.2% vs 97.2, 3.2x vs 3.2)
  ...
  // Check model / dataset indicators
  const modelSpecified = /\b(model|llama|gpt|bert|transformer|resnet|diffusion|architecture|agent|parameters|7b|70b)\b/i.test(
    `${claim} ${exactSupport}`,
  );
  const datasetSpecified = /\b(dataset|benchmark|mmlu|gsm8k|humaneval|imagenet|glue|squad|test set|evaluation)\b/i.test(
    `${claim} ${exactSupport}`,
  );
...
export function auditContextMismatch(claim: string, item: Partial<ClaimEvidenceLedgerItem>): {
  ...
  const datasetContext = (item.dataset || "").toLowerCase();
